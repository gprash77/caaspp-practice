...
function expectedCount(testNum: number, subject: "math" | "ela", testType: "cat" | "pt"): number {
  ...
  if (testNum >= 2 && testNum <= 18 && subject === "math" && testType === "pt") {
    return 5;
  }

  if (testNum >= 2 && testNum <= 18 && subject === "ela" && testType === "cat") {
    return 30;
  }

  if (testNum >= 2 && testNum <= 18 && subject === "ela" && testType === "pt") {
    return 3;
  }
...
describe("Question data integrity", () => {
  ...
  it("adds audio metadata to the rollout presentation clusters across the full presentation bank", () => {
    const rolloutChecks = [
      ...
      { testNum: 16, ids: [16118, 16119, 16121, 16122, 16130] },
      { testNum: 17, ids: [17118, 17119, 17121, 17122, 17130] },
      { testNum: 18, ids: [18119, 18120, 18121, 18122, 18130] },
    ];
